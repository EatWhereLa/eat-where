...
const modalValues: Ref<{ description?: string, location: string, priceLevel: string, reviews: Review[], openingHours: RestaurantDetails["current_opening_hours"] }> = ref({
  description: "",
  location: "",
  priceLevel: "",
  reviews: [],
    ...
    <div class="p-6 min-h-[220px] min-w-40 flex flex-col justify-between" >
      ...
      <va-card class="w-4/6 mx-auto p-4 mt-[-40px]">
          ...
          <va-card-content>
            ...
            <va-icon name="location_on" size="1.8rem" class="text-primary w-5 mr-2 float-left mb-3" />
            <p class="mt-1.5">{{ modalValues.location }}</p>
            <div class="clear-left"></div>

            <va-icon name="attach_money" size="1.8rem" class="text-primary w-4 mr-2 float-left mb-3" />
            <p class="mt-1.5">{{ modalValues.priceLevel }}</p>
            <div class="clear-left"></div>

            <va-icon name="alarm" size="1.8rem" class="text-primary w-4 mr-2 float-left mb-1" />
            ...
            <va-icon name="edit" size="1.8rem" class="text-primary w-5 mr-2 float-left" />
            <p v-if="modalValues.description !== undefined">{{ modalValues.description }}</p>
    ...
    <section class="mt-7">
        ...
        <va-accordion class="max-w-lg p-4">
          ...
          >
            <template #body>
              <li
                v-for="(review, index) in modalValues.reviews.sort((a,b) => b.time - a.time)"
                :key="index"
                class="list-none"
              >
                <ReviewItem
