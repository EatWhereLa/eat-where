...
const getRestaurantImageUrl = async (restaurant: RestaurantDetails, index: number) => {
...
const fetchBookings = async () => {
    ...
    bookingItems.value = await ky(
        URL,
    ).json() as typeof bookingItems.value;
};
...
const fetchPlaceInfos = async (): Promise<RestaurantDetails[]> => {
    ...
    return results.map((result, index) =>
        result.status === "fulfilled" && result.value ? result.value : bookingItemValues.value[index]
    );
};

const cancelBooking = async (event: { stopPropagation: () => void }, placeId: string) => {
    ...
    try {
        ...
        const bookingIndex = bookingItems.value.findIndex((item) => item.place_id === placeId);
        if (bookingIndex !== -1) {
...
<template>
    <div>
        <va-modal v-model="showModalValues.show" hide-default-actions class="mx-auto" size="large" closeButton fullscreen>
            <RestaurantModal :title="showModalValues.title" :place-id="showModalValues.placeId"
                :img-src="showModalValues.imgSrc" @closeModal="handleModal('', '', '')" />
        </va-modal>
